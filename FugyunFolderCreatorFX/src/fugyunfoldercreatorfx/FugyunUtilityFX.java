...
public class FugyunUtilityFX
{
	 ...
	 * ボタンテキストリスト（質問）
	 */
	public static final List<String> BUTTON_TEXT_LIST_QUSTION = List.of("はい", "いいえ");
	 ...
	 * @param stage ステージ
	 */
	public FugyunUtilityFX(Stage stage)
	{
		primaryStage = stage;
	}
	...
	public void FolderCreateTran(String pathText)
	{
		...
		try
		{
			Optional<ButtonType> result = new CustomMessageBox().showMessageBox(
					primaryStage,
					"フォルダ作成確認",
					"入力されたパスでフォルダを作成してよろしいですか？",
					AlertType.CONFIRMATION,
					BUTTON_TEXT_LIST_QUSTION,
					1);
			if (YES == result.get().getText())
			{
				// 未入力チェック
				if (pathText.isBlank())
				{
				...
				for (int index = 0; index < pathAr.length; index++)
				{
					...
					if (!path.isBlank())
					{
						...
						// チェック処理を行い、結果を判定する。
						if (CheckTran(primaryStage, path, driveLetterList, index + 1))
						{
	...
	public void FolderDeleteTran(boolean allDeleteFlg, String pathText)
	{
		...
		try
		{
			...
			String message = "入力されたパスのフォルダを削除してよろしいですか？\n（フォルダ内にファイルが存在する場合、ファイルごと削除します。）";
	...
	/**
	 * チェック処理
	 * @param ownerStage ステージ
	 * @param path パス
	 * @param driveLetterList ドライブ文字リスト
	 * @param rowIndex 行数
	 * @return チェック結果（true：正常・false：エラー）
	 */
	private boolean CheckTran(Stage stage, String path, List<String> driveLetterList, int rowIndex)
	{
		// 文字数チェック
		if (240 < path.length())
		{
			new CustomMessageBox().showMessageBox(
					primaryStage,
					Const.ERROR,
					convertNumberWide(rowIndex) + "行目：パスの入力可能文字数は、２４０文字までです。\n入力文字数：" + convertNumberWide(path.length()) + "文字",
					AlertType.ERROR,
					Const.BUTTON_TEXT_LIST_OK_ONLY,
					0);
			return false;
